...
%
  ...
  for cc=1:numel(template)
    ...
    sprintfstr = template{cc}(1:sidx(1)-1);
    ...
    for ii=1:numel(sidx);
      ...
      [vars{ii,2},vars{ii,3}] = strtok(template{cc}(ss+2:ee-2), '%');
      ...
      if ii ~= numel(sidx)
        nextpart = template{cc}(ee+1:sidx(ii+1)-1);
      else
        nextpart = template{cc}(ee+1:end);
      end
    ...
    str{cc} = sprintf(sprintfstr, vars{:,4});
  end
