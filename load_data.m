...
%    for the corresponding data product loading. If an extra parameter is
%    needed, remember to fill in VARS with either {'*'} or [] to load all
%    data or the list of desired variables.
%
%        calval
%            None
%        map
%            1. coaddopt
%               Data structure with same form as passed into the
%               REDUC_COADDPAIRMAP function. The field SERNUM must contain a
...
%
    ...
    switch product
        ...
        case 'map' % {{{
            ...
            % We require at least a serial number to exist within the coaddopt
            % struct since it is always a requirement
            coaddopt = varargin{1};
            ...
            % If there is more than one jacktype at this stage, we also must
            % bail since we don't know which one we should load.
            if length(coaddopt.jacktype) > 1
                error(['load_data [map]: Only one jackknife type '...
                       '(coaddopt.jacktype) can be given.']);
            end

            % Then build the strings required to build the full path. Taken
            % directly from reduc_coaddpairmaps
