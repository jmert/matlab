...
%                  This method tries to identify complete runs of the telescope
%                  and groups the tags together to comprise a complete run. For
%                  BICEP2, this should be 67 tags per group in the 2012 season,
%                  but various errors can make this number fluctuate. Sets are
%                  identified by grouping contiguous runs of a single deck
%                  angle within the list of tags returned by GET_TAGS.
%
%               2. binned
%                  Simple binning where each block of N tags are taken
%                  together. The last group returned may have a variable number
%                  of tags in the group since the total number of tags may not
%                  be evenly divisible by N. See the EXTRA option for more
%                  information.
%
%               3. rsrn
%                  Takes the ratio of the operating resistance of the detectors
%                  versus their resistance in the normal state to derive the
%                  ratio r = R_s / R_n. The value is derived from reading in
%                  all corresponding *_calval.mat files.  Once collected, the
%                  tags are then sorted by r and binned according to the value
%                  passed into EXTRA.
%
...
%
%               2. binned
%                  Scalar integer expected which prescribes N, the number of
%                  tags to be placed in each bin.
%
%               3. rsrn
...
%
%OUTPUT
%  GROUPS     A cell array with each element a group. Each group is thenu cell
%             array of the constituent tags.
...
%                     List of tags removed because r > 0.
%                  e. rsrn_groups
%                     Cell array of groups of R_s/R_n values corresponding to
%                     each group output in GROUPS.
%                  f. indx_groups
  ...
  switch divtype
    case 'perdaydeck' % {{{
      while length(tags) > 0
        ...
        % Then produce a mask of all tags which match the given deck angle.
        matches = ~cellfun(@isempty, strfind(tags, deck));
        % Now adapt the logic of find_blk to get the first contiguous block of
        % tags.
        ...
        % Also remember to set the unique string. Since there are no options
        % here, just return 'perdaydeck'
        uid = 'perdaydeck';
      end
    % }}}
    case 'binned' % {{{
      ...
      % Construct the unique string from 'binned' + the number of tags combined
      % for each bin.
      uid = sprintf('binned%i', extra);
    % }}}
    case 'rsrn' % {{{
      % First verify that our inputs are correct. No use in continuing if the
      % operation will fail later.
      if isempty(extra)
      ...
      if ~exist('grouptags_rsrn.mat','file')
        ...
        % Otherwise if it does, see if we can short-circuit this step by first
        % loading only the tags and seeing if they correspond with each other.
        % If they do, then we can again assume the data is OK.
        saved = load('grouptags_rsrn.mat','tags');
        % Compare the lengths of each string first, then if they are the same,
        % continue by comparing each string. If no match returns a false, then
      ...
      % therefore don't have two rgl_rsrn values). Also save the problem tags
      ...
      % Sort the R_s/R_n values so that we can division them into equally sized
      % groups.
      ...
      % Also reuse the machinery of bintags to create corresponding groups of
      % the R_s/R_n values and their indices into the allrsrn array
